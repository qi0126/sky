...
export default {
  ...
  methods: {
    created_fun() {
      var self = this;
      this.$axios
        .get(this.portUrl + "/batar-sjy-manage/stock/receivematerial/order/list")
        .then(function(res) {
          if (res.data.code == 200) {
            ...
            for (var i = 0; i < self.receiveMaterData.length; i++) {
              //日期格式化
              self.receiveMaterData[i].receive_material_time = self.$api.dateGetDay(
    ...
    editGold(elem) {
      var self = this;
      console.log(elem)
      this.$router.push({ path: "editReceiveMater", query: { editId: elem.id } });
    },
    //创建新收料单
    createReceive() {
      var self = this;
