...
export default {
  ...
  methods: {
    created_fun() {
      ...
      this.$axios
        .get(
          this.portUrl + "/batar-sjy-manage/stock/inputstock/order/list",
        )
        .then(function(res) {
          if (res.data.code == 200) {
            ...
            for(var i=0 ; i<self.receiveMaterData.length ; i++){
              ...
              self.receiveMaterData[i].create_time = self.$api.dateGetDay(self.receiveMaterData[i].create_time)
            }
    ...
    viewGold(elem) {
      var self = this;
      this.$router.push("stockEnterEdit?view&goldId=" + elem.id);
    },
    //点击编辑收料单
    editGold(elem) {
      var self = this;
      this.$router.push({path:'stockEnterEdit',query:{editId:elem.id}});
      
    },
    //创建新收料单
    createReceive(){
      var self = this
      this.$router.push({path:'stockEnterEdit',query:{editId:'createData'}})
    }
  }
};
</script>
